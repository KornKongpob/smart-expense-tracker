...
import { X, Check, ChevronRight, ChevronDown, Camera, FileText } from "lucide-react";
...
export default function QuickAddSheet({ isOpen, onClose }) {
  ...
  const selectedSub = subCats.find((c) => c.id === subCatId);
  const displayCats = showAllCats ? mainCats : mainCats.slice(0, 8);

  return (
    <div className="fixed inset-0 z-[55] flex items-end justify-center">
      <button type="button" className="absolute inset-0 bg-black/50 backdrop-blur-sm" onClick={onClose} aria-label="Close" />

      <div className="relative w-full max-w-md rounded-t-[28px] p-5 pb-safe animate-fade-in-up bg-white/95 backdrop-blur-2xl border-t border-white/40 shadow-[0_-20px_60px_rgba(0,0,0,0.15)]">
        {/* Drag handle */}
        ...
        <div className="flex gap-1.5 p-1 rounded-2xl bg-gray-100 mb-5">
          {[
            ...
            <button
              key={t.key}
              type="button"
              onClick={() => { setType(t.key); setMainCatId(""); setSubCatId(""); }}
            ...
            <>
              ...
              {mainCats.length > 8 && (
                <button
                  ...
                  className="mt-2 w-full text-center text-xs font-extrabold text-indigo-600 py-1.5 active:scale-95"
                >
                  {showAllCats ? "แสดงน้อยลง" : `ดูทั้งหมด (${mainCats.length})`} <ChevronDown size={12} className={`inline transition-transform ${showAllCats ? "rotate-180" : ""}`} />
                </button>
              )}
            </>
          ) : (
