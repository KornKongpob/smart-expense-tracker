...
function normalizeSnapshot(snapshot) {
  ...
  return {
    ...
    financialGoals: toArray(data.financialGoals ?? data.financial_goals ?? data.goals),
...
async function upsertFinancialGoals(admin, userId, financialGoals, accountIdMap) {
  const rows = toArray(financialGoals).map((goal) => ({
    ...
    target_amount_satang: Math.max(0, toInt(goal?.target_amount_satang ?? goal?.targetAmountSatang ?? goal?.targetAmount, 0)),
    current_amount_satang: Math.max(0, toInt(goal?.current_amount_satang ?? goal?.currentAmountSatang ?? goal?.currentAmount, 0)),
    target_date: sanitizeOptionalDate(goal?.target_date ?? goal?.targetDate ?? goal?.dueDate),
    monthly_contribution_satang: Math.max(
      0,
      toInt(goal?.monthly_contribution_satang ?? goal?.monthlyContributionSatang ?? goal?.monthlyContribution, 0),
    ),
    linked_account_id: mapAccountId(
      goal?.linked_account_id ??
        goal?.linkedAccountId ??
        (Array.isArray(goal?.linkedAccountIds) ? goal.linkedAccountIds[0] : null),
      accountIdMap,
    ),
