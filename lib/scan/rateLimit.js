...
export function createRateLimiter({
  ...
  return function rateLimit(req, res) {
    const configuredLimit = limit ?? limitPerMinute ?? Number(process.env.SCAN_RATE_LIMIT_PER_MINUTE || 30);
    const cap = Number.isFinite(configuredLimit) && configuredLimit > 0 ? configuredLimit : 0;
    if (!cap) return true;
