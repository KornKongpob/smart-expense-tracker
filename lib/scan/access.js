...
export async function enforceAccess(req, res) {
  ...
  // A valid shared token is the server-to-server path and stands on its own.
  if (tokenOk) return true;

  // The origin allowlist only filters browsers: an Origin header is trivial to
  // set from a script, so it narrows who can reach the endpoint but never proves
  // who the caller is. It is a gate to pass, not a credential.
  if (originsConfigured && !originOk) {
    res.status(403).json({ ok: false, code: "forbidden", message: "Origin not allowed" });
    return false;
  }

  if (tokenConfigured && !originsConfigured) {
